...
public class ExceptionHandling {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * << 런타임 에러 >>
		 * - 런타임 에러 방생시 발생한 위치에서 프로그램이 비정상적으로 종료된다.
		 * - 에러 : 프로그램 코드에 의해서 수습될 수 없는 심각한 오류 (처리 불가)
		 * - 예외 : 프로그램 코드에 의해서 수습될 수 있는 다소 미약한 오류 (처리가능)
		 * 
		 * << 예외 >>
		 * - 모든 예외는 Exception 클래스의 자식 클래스이다.
		 * - RuntimeException 클래스와 그 자식들을 예외처리가 강제되지 않는다. (unchecked)
		 * - [RuntimeException 클래스와 그 자식들을 제외한] Exception 클래스 예외처리가 강제된다. (checked 제외)
		 * 
		 * << 에외 처리(try-catch) >>
		 ...
		 * - 발생한 예외와 일치하는 catch 블럭 안의 내용이 수행한 후 try-catch를 빠져나간다.
		...
		try{
			...
			//예외 발생시 발생한 예외와 일치하는 catch블럭이 수행 되고
